...
from model import Linear, BatchNorm1d, Tanh, Embeddings, FlattenConsecutive, Sequential
...
def build_neural_net(vocab_size):
    model = Sequential([
        Embeddings(N_EMBED, vocab_size),
...
def train_model(X_tr, Y_tr, model):
...
def execute():
    ...
    model = build_neural_net(vocab_size)
    train_model(X_tr, Y_tr, model)

    # put layers into eval mode
    for layer in model.layers:
        layer.training = False

    evaluate_loss('dev', X_dev, Y_dev, model)
    evaluate_loss('test', X_test, Y_test, model)
    
    generated_names = inference(model, i_tos)
...
def evaluate_loss(split, x, y, model):
    logits = model(x)
